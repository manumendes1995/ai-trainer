...
function setTip(t){ tipEl && (tipEl.textContent=t); lastTipAt=performance.now(); }
function nowSec(){ return Math.floor((performance.now()-startAt)/1000); }

// ===== Câmara com resolução dinâmica (adaptativa)
const RES_STEPS = [
  {w:1280,h:720,label:"HD"},
  {w:960,h:540,label:"qHD"},
  {w:640,h:480,label:"SD"},
];
let resIdx = 0;               // começa em HD
let perfAcc=0, perfFrames=0;  // mede desempenho
...
function takePhoto(){
  ...
  const a = document.createElement("a"); a.href=url; a.download=`ai-trainer-foto-${Date.now()}.png`; a.click();
...
function toggleRecord(){
  ...
  if(!recording){
    ...
    mediaRecorder.onstop=()=>{
      ...
      btnDownload.disabled=false; btnDownload.onclick=()=>{ const a=document.createElement("a"); a.href=url; a.download=`ai-trainer-video-${Date.now()}.webm`; a.click(); URL.revokeObjectURL(url); };
      setTip("Gravação concluída. Clique “Guardar Vídeo”.");
    };
    mediaRecorder.start(); recording=true; btnRecord.textContent="⏹️ Parar gravação"; setTip("A gravar vídeo…");
  }else{
    mediaRecorder?.stop(); recording=false; btnRecord.textContent="● Gravar";
  }
}
...
async function loop(){
  ...
  // Ajuste simples: se média dos últimos ~30 frames > 80ms, baixa resolução
  if (perfFrames>=30){
    const avg = perfAcc/perfFrames;
    if (avg>80 && resIdx < RES_STEPS.length-1){
      resIdx++;
      setTip(`FPS baixo: a reduzir para ${RES_STEPS[resIdx].label}…`);
      stopCamera(); await startCamera(); // reabre com nova resolução
    }
...
function draw(poses){
  ...
  const SHOW = (typeof window.__SHOW_SKEL!=="undefined") ? window.__SHOW_SKEL : true;
  if (SHOW){
    ctx.fillStyle="#00e676";
    kp.forEach(p=>{ if(p.score>0.5){ ctx.beginPath(); ctx.arc(p.x,p.y,3,0,Math.PI*2); ctx.fill(); } });
  }

  // Contagem simples: braço direito (exemplo)
  const rW = kp.find(p=>p.name?.includes("right_wrist"));
  const rS = kp.find(p=>p.name?.includes("right_shoulder"));
  if(exerciseSel.value==="rightArm" && rW && rS){
    if(rW.y+30<rS.y && phase==="down"){ phase="up"; }
    if(rW.y>rS.y+30 && phase==="up"){ phase="down"; reps++; repsEl.textContent=reps; maybeGoal(); setTip("Boa! Continua 💪"); }
  }
  if(exerciseSel.value==="leftArm" && performance.now()-lastTipAt>3000){ setTip("Levanta e baixa o braço esquerdo até ao ombro."); }
...
function saveSession(){
  ...
  arr.push(item); localStorage.setItem("ai.trainer.hist", JSON.stringify(arr)); setTip("Sessão guardada 💾");
}
function showHistory(){
  ...
  arr.forEach(i=>{ html+=`<tr><td>${new Date(i.ts).toLocaleString()}</td><td>${i.exercise}</td><td>${i.reps}</td><td>${i.goal}</td><td>${i.duration}</td></tr>`; });
  html+="</tbody></table>"; historyTable.innerHTML=html;
}
...
function exportCSV(){
  ...
  const a=document.createElement("a"); a.href=url; a.download="ai-trainer-historico.csv"; a.click(); URL.revokeObjectURL(url);
}
...
btnStart.addEventListener("click", async ()=>{
  try{
    ...
    setStatus("A abrir câmara…"); await startCamera(); await initTF(); startTimer(); loop(); setTip("Treino iniciado ✅");
  }catch(e){ console.error(e); setTip("Erro ao iniciar: "+e.message); }
});
btnStop.addEventListener("click", ()=>{ stopTimer(); stopCamera(); cancelAnimationFrame(raf); setStatus("Parado ⏹️"); setTip("Treino parado."); });
